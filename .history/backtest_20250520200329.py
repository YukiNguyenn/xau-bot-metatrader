...
class Backtest:
    ...
    def check_symbol(self, symbol: str) -> bool:
        ...
        try:
            ...
            # Đăng nhập
            if mt5.login(login=204596757, password="12345678aA@", server="Exness-MT5Trial7"):
                print("Đăng nhập thành công")
            else:
                print("Đăng nhập thất bại, mã lỗi =", mt5.last_error())
            # Get all symbols
    ...
    def download_data(self, days=300):
        ...
        # Initialize MT5 with more detailed error handling
        if not mt5.initialize(
            path="C:\\Program Files\\MetaTrader 5\\terminal64.exe",  # Path to MT5 terminal
        ...
        try:
            ...
            if not mt5.terminal_info():
                error = mt5.last_error()
                self.logger.error(f"Not connected to MT5 terminal. Error code: {error}")
                return False
            ...
            if symbol_info is None:
                ...
                self.logger.error("Trying to select symbol first...")
                
                # Try to select symbol first
                if not mt5.symbol_select(symbol, True):
                    error = mt5.last_error()
                    self.logger.error(f"Failed to select symbol {symbol}. Error code: {error}")
                    return False
                ...
                if symbol_info is None:
                    error = mt5.last_error()
                    self.logger.error(f"Still failed to get symbol info after selection. Error code: {error}")
                    return False
