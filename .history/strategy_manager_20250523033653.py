...
class StrategyManager:
    ...
    def add_strategy(self, strategy_name: str):
        ...
        self.logger.info(merged_config)

        # Import và khởi tạo class từ tên
        if strategy_name == "rsi_strategy":
            strategy_instance = RSIStrategy(merged_config)
        else:
    ...
    def _run_strategy(self, strategy: BaseTradingStrategy):
        ...
        while self.running:
            try:
                ...
                # Check for signals
                signals = strategy.check_signals(data)
                
                # Process signals
                for signal in signals:
                    # Check risk management
                    if not self.risk_manager.can_open_position(signal['volume'], signal['price']):
                        continue
                    ...
                    if order_id:
                        ...
                        self.risk_manager.update_open_positions(
                            len(self.trade_manager.get_open_positions())
                        )
                        
                        # Log trade
                        strategy.add_trade_log({
                            'order_id': order_id,
                            'type': order_type,
                            'volume': volume,
                            'price': price,
