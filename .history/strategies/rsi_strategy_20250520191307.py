...
class RSIStrategy(BaseTradingStrategy):
    ...
    def check_signals(self, data: pd.DataFrame) -> list:
        """Check for trading signals based on RSI values"""
        signals = []
        
        # Calculate RSI for all timeframes
        rsi_short = self.calculate_rsi(data, self.rsi_periods['short'])
        rsi_medium = self.calculate_rsi(data, self.rsi_periods['medium'])
        rsi_long = self.calculate_rsi(data, self.rsi_periods['long'])
        ...
        if (current_rsi_short <= self.rsi_levels['short']['oversold'] and
            ...
            signals.append({
                ...
                'sl': data['close'].iloc[-1] - self.config['strategy']['parameters']['risk_management']['stop_loss_pips'] * 0.1,
                'tp': data['close'].iloc[-1] + self.config['strategy']['parameters']['risk_management']['take_profit_pips'] * 0.1
            })
