...
class Backtest:
    ...
    def run_backtest(self, strategy_class=RSIStrategy):
        ...
        # Use M5 as main timeframe to match strategy
        main_tf = 'M5'
        ...
        for i in range(1, len(main_data)):
            ...
            current_data = {tf_name: df[df['time'] <= current_time].copy() for tf_name, df in data.items()}
            ...
            for signal in signals:
                ...
                pip_distance = abs(signal['sl'] - signal['price']) / point
                position_size = min(
                ...
                self.logger.info(f"Opened {signal['type']} trade: Price={signal['price']:.2f}, Volume={position_size:.2f}, SL={signal['sl']:.2f}, TP={signal['tp']:.2f}")
            ...
            for trade in open_trades[:]:
                ...
                if trade_type == 'BUY':
                    if current_price >= tp:
                        profit = (tp - entry_price) / point * pip_value * volume
                        ...
                        loss = (entry_price - sl) / point * pip_value * volume
                    ...
                    if current_price <= tp:
                        profit = (entry_price - tp) / point * pip_value * volume
                        current_balance += profit
                        results['trades'].append({
                            **trade,
                            'exit_price': tp,
                        ...
                        loss = (sl - entry_price) / point * pip_value * volume
                        current_balance -= loss
                        results['trades'].append({
                            **trade,
                            'exit_price': sl,
        ...
        self.logger.info(f"Total trades: {results['metrics']['total_trades']}")
        self.logger.info(f"Win rate: {results['metrics']['win_rate']:.2%}")
        self.logger.info(f"Profit factor: {results['metrics']['profit_factor']:.2f}")
        self.logger.info(f"Max drawdown: {results['metrics']['max_drawdown']:.2%}")
